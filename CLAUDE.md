...
- **The 5h reset clock**: in the red the simple 5h form appends the wall-clock reset time (`format_clock`, `date -r` on macOS vs `date -d @` elsewhere). It shares the `$RED_PCT` threshold with `color_by_pct`, so the clock appears exactly when the segment turns red.
- **The `avg` pace segments are colored by pace, not fill**: both the 7d pace (in `add_window_segment`) and the monthly pace color by the `avg/left` ratio (green <1.0, yellow <1.5, red ≥1.5), computed in the same awk that formats the pace, *not* by `color_by_pct` on the window/month %. This is deliberate — don't "unify" it with the neighboring segment's color. The monthly short form (`avg $X/d` alone — month end or budget gone) has no `left` rate to compare against, so awk emits an empty color code and the segment falls back to `color_by_pct` on the month fill.
