...
- **Cents, not dollars**: `extra_usage.monthly_limit`/`used_credits` are in cents despite a `currency` field; the script divides by 100. `currency` is not always `USD` (EUR-billed accounts report `EUR`) but the script always renders `$`.
- **`extra_usage` is not Enterprise-only**: Pro/Max accounts with pay-as-you-go credits also report `is_enabled: true`, but with `utilization: null`. That null is the discriminator between the two M-segment forms — Enterprise (utilization present) gets the full `$spent Nd used PCT% bar $left Md left` + pace, Pro gets only `M: $X left` (no bar, no pace, no calendar days, which is why the day/pace computation sits inside the `[ -n "$month_pct" ]` branch). The Pro form is colored by `used_credits/monthly_limit`, since there's no utilization to color by. Hence `utilization // ""` in the jq read — a `// 0` default would silently render Enterprise's form with a fake 0%.
